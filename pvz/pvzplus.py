...
#植物信息包括选卡界面行列，冷却时间，生效时间
class PlantData:
    def __init__(self,coord,cd,effectdelay):
        self.coord=coord
        self.cd=cd
        self.effectdelay=effectdelay

CD={'fast':750,'slow':3000,'veryslow':5000}

PlantsData={'CherryBomb':PlantData((1,3),CD['veryslow'],100),\
            'WallNut':PlantData((1,4),CD['slow'],0),\
            'PotatoMineTooltip':PlantData((1,5),CD['slow'],1500),\
            'PuffShroom':PlantData((2,1),CD['fast'],0),\
            'SunShrooom':PlantData((2,2),CD['fast'],0),\
            'FumeShrooom':PlantData((2,3),CD['fast'],0),\
            'GraveBuster':PlantData((2,4),CD['fast'],454),\
            'ScaredyShroom':PlantData((2,6),CD['fast'],0),\
            'IceShroom':PlantData((2,7),CD['veryslow'],100),\
            'DoomShroom':PlantData((2,8),CD['veryslow'],100),\
            'LilyPad':PlantData((3,1),CD['fast'],0),\
            'Squash':PlantData((3,2),CD['slow'],181),\
            'Jalapeno':PlantData((3,5),CD['veryslow'],100),\
            'Spikeweed':PlantData((3,6),CD['fast'],0),\
            'TallNut':PlantData((3,8),CD['slow'],0),\
            'Blover':PlantData((4,4),CD['fast'],0),\
            'Pumpkin':PlantData((4,7),CD['slow'],0),\
            'FlowerPot':PlantData((5,2),CD['fast'],0),\
            'CoffeeBean':PlantData((5,4),CD['fast'],298),\
            'Shovel':PlantData(None,0,0),\
            'Pao':PlantData(None,3475,373),\
            'Imitater':PlantData(None,None,320)}
...
#选择单张卡片
def SelectSeed(seed,isimitater):
    row,col = PlantsData[seed].coord
    _select_seed_by_crood(row, col, isimitater)

#可用植物包括选中坐标，冷却时间，生效时间
class Plant:
    def __init__(self,startpoint,cd,effectdelay):
        self.startpoint=startpoint
        self.cd=cd
        self.effectdelay=effectdelay
        #初始状态为冷却完毕
        self.usingtime=_game_clock()-cd

#先点选中坐标，再点使用坐标使用植物
    def UsePlant(self,grid):
        endpoint=GridToPoint(grid)
        _mouse_lock.acquire()
        _safe_click()
        _mouse_left_click(*self.startpoint)
        _mouse_left_click(*endpoint)
        _mouse_lock.release()
        self.usingtime=_game_clock()

#根据OT轴使用植物
    @_running_in_thread
    def UsePlantByOperateTime(self,operatetime,grid):
        self.SetUsingTime(operatetime)
        _game_delay_for(operatetime+200)
        self.UsePlant(grid)

#根据ET轴使用植物
    def UsePlantByEffectTime(self,effecttime,grid):
        self.UsePlantByOperateTime(effecttime-self.effectdelay,grid)

    def SetUsingTime(self,operatetime):
        self.usingtime=_game_clock()+200+operatetime

    def IsReady(self,operatetime):
        return _game_clock()+200+operatetime-self.usingtime>=self.cd

class SelectedCard(Plant):
    def __init__(self,index,card):
        self.isimitater=False
        if card[0:2]=='im':
            self.isimitater=True
            card=card[2:]
        Plant.__init__(self,IndexToPoint(index),\
                       PlantsData[card].cd,PlantsData[card].effectdelay)
        #初始化时完成选卡
        SelectSeed(card,self.isimitater)
        #模仿植物生效时间加上模仿者生效时间
        if self.isimitater:
            self.effectdelay+=PlantsData['Imitater'].effectdelay
        #白天蘑菇生效时间加上咖啡豆生效时间，垫材不算
        if _game_scene in [0,2,4] and card in ['IceShroom','DoomShroom']:
            self.effectdelay+=PlantsData['CoffeeBean'].effectdelay

class SetPao(Plant):
    def __init__(self,grid):
        Plant.__init__(self,GridToPoint(grid),\
                       PlantsData['Pao'].cd,PlantsData['Pao'].effectdelay)

    #发射炮要确保选中生效和使用生效，有问题（会把endoint的炮也发射了）
    '''
    def UsePlant(self,grid):
        endpoint=GridToPoint(grid)
        _mouse_lock.acquire()
        _safe_click()
        while not _mouse_have_something():
            print('startclick: time:%s grid:%s' %(_game_clock(),self.startpoint))
            _mouse_left_click(*self.startpoint)
            _delay_a_little_time()
        while _mouse_have_something():
            print('endclick: time:%s grid:%s' %(_game_clock(),endpoint))
            _mouse_left_click(*endpoint)
            _delay_a_little_time()
        _mouse_lock.release()
        self.usingtime=_game_clock()
...
class MushroomList:
    def __init__(self,mushroom,mushroomlist,layout):
        self.layout=layout
        self.mushroom=mushroom
        self.mushroomlist=mushroomlist
        self.num=len(mushroomlist)
        self.head=0
        self.tail=0

    #使用蘑菇
    def UseMushroom(self,effecttime):
        call=_game_scene in [0,2,4]
        self.layout.UsePlantByEffectTime(\
            self.mushroom,effecttime,self.mushroomlist[self.tail%self.num],call)
        self.tail+=1

    #存蘑菇
    def FillMushroom(self):
        self.layout.CardList[self.mushroom].UsePlant(\
            self.mushroomlist[self.head%self.num])
        self.head+=1
        if self.layout.imitater == self.mushroom:
            self.layout.CardList['im'+self.mushroom].UsePlant(\
                self.mushroomlist[self.head%self.num])
            self.head+=1

    #唤醒存的蘑菇
    def CallMushroom(self,effecttime):
        self.layout.CardList['CoffeeBean'].UsePlantByEffectTime(\
            effecttime,self.mushroomlist[self.tail%self.num])
        self.tail+=1
    ...
    #自动存蘑菇
    @_running_in_thread
    def ThreadingFill(self):
        while _game_ui() == 3 and self.layout.wave<self.layout.filluntil:
            while _game_paused():
                _game_delay_for(1)
            self.FillMushroom()
            _game_delay_for(5010)

        

class Layout:
    def __init__(self,PaoList,CardList,\
                 IceList,DoomList,\
                 UnplantableList,EatableList):
        self.PaoList=list(map(SetPao,PaoList))
        self.paonum=len(PaoList)
        self.nowpao=0
        self.IceList=MushroomList('IceShroom',IceList,self)
        self.DoomList=MushroomList('DoomShroom',DoomList,self)
#        self.SideDoomList=SideDoomList
#        self.sidedoomnum=len(SideDoomList)
#        self.sidedoomhead=0
#        self.sidedoomtail=0
        self.CardList=dict(map(\
            lambda index,card:(card,SelectedCard(index,card)),range(1,11),CardList))
        self.imitater=None
        self.useim=False
        for card in CardList:
            if card[0:2]=='im':
                self.imitater=card[2:]
        self.CardList['Shovel']=Plant((640,20),0,0)
        #在以下位置使用植物需要先种睡莲或花盆
        self.UnplantableList=UnplantableList
        #在以下位置使用植物需要先种南瓜防啃
        self.EatableList=EatableList

    #在同一时间使用的的多张卡有先后次序要求，
    #比如白天在水路使用毁灭菇要按睡莲，毁灭菇，咖啡豆的顺序
    @_running_in_thread
    def UseCardsByOperateTime(self,plants,operatetime,grid):
        for plant in plants:
            self.CardList[plant].SetUsingTime(operatetime)
        _game_delay_for(operatetime+200)
        for plant in plants:
            if plant[0:2]=='im':
                print(_game_clock())
            self.CardList[plant].UsePlant(grid)
        
    #自动判断是否需要先种睡莲花盆南瓜，call=True表示需要唤醒
    def UsePlantByEffectTime(self,plant,effecttime,grid,call=False):
        if plant=='Pao':
            self.PaoList[self.nowpao%self.paonum].UsePlantByEffectTime(effecttime,grid)
            self.nowpao+=1
        else:
            plants=[]
            if grid in self.UnplantableList:
                if _game_scene in [2,3]:
                    plants.append('LilyPad')
                elif _game_scene in [4,5]:
                    plants.append('FlowerPot')
            if grid in self.EatableList:
                plants.append('Pumpkin')
            useim=False
            if plant==self.imitater:
                useim=self.useim
                if self.useim:
                    plant='im'+plant
                self.useim=not self.useim
            plants.append(plant)
            if call:
                if useim:
                    self.CoffeeBean(effecttime,grid)
                else:
                    plants.append('CoffeeBean')
            self.UseCardsByOperateTime(\
                plants,effecttime-self.CardList[plant].effectdelay,grid)

    #确保咖啡豆在模仿者生效后使用
    @_running_in_thread
    def CoffeeBean(self,effecttime,grid):
        operatetime=effecttime-100-PlantsData['CoffeeBean'].effectdelay
        self.CardList['CoffeeBean'].SetUsingTime(operatetime)
        _game_delay_for(operatetime+200)
        endpoint=GridToPoint(grid)
        _mouse_lock.acquire()
        _safe_click()
        _mouse_left_click(*self.CardList['CoffeeBean'].startpoint)
        print(_game_clock())
        while _mouse_have_something():
            print(_game_clock())
            _mouse_left_click(*endpoint)
...
class Game(Layout):
    def __init__(self,PaoList,CardList,\
                 ...
                 DoomList,filleddoom,\
                 UnplantableList,EatableList,\
                 eachwave,waveDealPao=0,filluntil=21):
        Layout.__init__(self,PaoList,CardList,\
                        IceList,DoomList,\
                        UnplantableList,EatableList)
        #-1表示不存蘑菇，非负整数表示已存蘑菇的数量
        if filledice == -1:
            self.icefill=False
        else:
            self.icefill=True
            self.IceList.head=filledice
        if filleddoom == -1:
            self.doomfill=False
        else:
            self.doomfill=True
            self.DoomList.head=filleddoom
        self.operates={}
        self.operates['P']=functools.partial(self.UsePlantByEffectTime,'Pao')
        self.operates['A']=self.UsePlantByEffectTime
        #根据存蘑菇策略决定I,N是唤醒还是使用蘑菇
        self.operates['I']=self.IceList.CallMushroom\
                            if self.icefill else \
                            self.IceList.UseMushroom
        self.operates['N']=self.DoomList.CallMushroom\
                            if self.doomfill else \
                            self.DoomList.UseMushroom
                            
        self.wave=0
        self.eachwave=eachwave
        self.waveDealPao=waveDealPao
        self.filluntil=filluntil
    ...
    def Start(self):
        ...
        if self.icefill:
            self.IceList.ThreadingFill()
        if self.doomfill:
            self.DoomList.ThreadingFill()
        self.WaveLoop()
    ...
    def WaveLoop(self):
        ...
        while self.wave<20:
            ...
            for effect in self.eachwave[self.wave-1]:
                print('effect: %s' % list(effect))
                self.operates[effect[0]](*effect[1:])
            Delay(600)
            if self.wave%10==9:
                self.SkipPao()
            if self.wave<20:
                Countdown(200,self.wave%10==9)


    @_running_in_thread
    def SkipPao(self):
        #nowwave=self.wave
        #Delay(int(3475*self.waveDealPao/self.paonum))
        #if nowwave==self.wave:
        self.nowpao+=self.waveDealPao
...
Wavecountdown = _wave_countdown
Hugewavecountdown = _huge_wave_countdown
